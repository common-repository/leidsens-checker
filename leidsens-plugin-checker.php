...
Tested up to: 6.4.2
Requires PHP: 7.2
Text Domain: leidsens2019
Version: 1.0.0
*/

if ( ! defined( 'ABSPATH' ) ) exit; // Exit if accessed directly      
// namespace leidsens_plugin_checkers;
function LTPCD_refreshPage(){
	?>
...
if(!defined('LTPCD_PLUGIN_CHECKER_VERSION')){
    define('LTPCD_PLUGIN_CHECKER_VERSION', '1.0.0');
}

function LTPCD_load_custom_wp_admin_style($hook) {
	if(($hook == 'toplevel_page_leidsensplugin_checker') || ($hook == 'leidsens-plugin-checker_page_urlreplace') || ($hook == 'leidsens-plugin-checker_page_disableeditor')) {
		...
		wp_enqueue_style( 'plugin_checker_select_style_lib', plugins_url('includes/assets/css/select2.min.css', __FILE__), false, null, 'all');
...
add_action( 'admin_enqueue_scripts', 'LTPCD_load_custom_wp_admin_style' );

function LTPCD_create_leidsens_plugin_checker_database_table()
{
  	global $wpdb;
...
register_activation_hook( __FILE__, 'LTPCD_create_leidsens_plugin_checker_database_table' );

/**
 * Register a custom menu page.
 */
function LTPCD_leidsens_register_checker_menu_page() { 
  	add_menu_page( 
		'Leidsens Plugin Checker',
		'Leidsens Plugin Checker',
		'manage_options',
		'leidsensplugin_checker',
		'LTPCD_leidsens_plugin_checker',
  	...
  	add_submenu_page(
		...
		'manage_options',
		'urlreplace',
		'LTPCD_leidsens_urlreplace_checker'
    );
    ...
    add_submenu_page(
		...
		'manage_options',
		'disableeditor',
		'LTPCD_leidsens_disable_editor'
    );
}
add_action('admin_menu', 'LTPCD_leidsens_register_checker_menu_page');

if ( ! class_exists( 'WP_List_Table' ) ) {
	require_once( ABSPATH . 'wp-admin/includes/class-wp-list-table.php' );
}

class LTPCD_Plugins_List extends WP_List_Table {
	/** Class constructor */
	...
	*/
	...
	function prepare_items() {
        ...
        $perpage = 5;
        //Which page is this?
        $paged = !empty($_GET["paged"]) ? mysqli_real_escape_string($wpdb, esc_html(sanitize_text_field($_GET["paged"]))) : '';
        //Page Number
 ...
 */
function LTPCD_leidsens_plugin_checker(){
	
	$redirects = 1;
	if (isset($_POST['leidsens_disable_plugin']) && !empty($_POST['leidsens_disable_plugin'])) {
		foreach ($_POST['plugins_to'] as $value) {
			$sanVal = sanitize_text_field($value);
			$escapeVal = esc_html($sanVal);
			deactivate_plugins( '/'.$escapeVal, true);		
		}
		$redirects = 2;
	}

	if (isset($_POST['leidsens_enable_plugin']) && !empty($_POST['leidsens_enable_plugin'])) {
	...
	if (isset($_POST['leidsens_disableupdate_plugin']) && !empty($_POST['leidsens_disableupdate_plugin'])) {
		global $wpdb;
		...
		foreach ($_POST['upd_plugins_to'] as $value) {
			...
			$data = array('time' => date('Y-m-d H:i:s'), 'plugin_name' => sanitize_text_field($pluginArr[0]), 'version_text' => sanitize_text_field($pluginArr[1]), 'disable_status' => 'Yes');
			$format = array('%s','%s','%s','%s');
			$wpdb->insert($table,$data,$format);
			$disablePlugin_id = $wpdb->insert_id;
		}
		$redirects = 2;
	}

	if (isset($_POST['leidsens_enableupdate_plugin']) && !empty($_POST['leidsens_enableupdate_plugin'])) {
		global $wpdb;
		$table = $wpdb->prefix . "leidsens_plugin_checker_updates";
		foreach ($_POST['en_upd_plugins_to'] as $value) {
			...
			$escapeVal = esc_html($sanVal);			
			$pluginArr = explode("_", $escapeVal);
			$wpdb->query( $wpdb->prepare("DELETE FROM $table WHERE plugin_name = %s AND version_text = %s", sanitize_text_field($pluginArr[0]), sanitize_text_field($pluginArr[1])));
		}
		$redirects = 2;
	}
	if ($redirects == 2) {
							...
							<div class="pluginChecker_multiSelect">
								...
								<select name="plugins_to[]" id="plugins_to" class="plugins_to" multiple="multiple" width="300px">
									<option>Select here...</option><?php foreach ($activated_plugins as $key=>$value) { ?>
											<option value="<?php echo wp_kses_post($key); ?>"><?php echo wp_kses_post($value['Name']); ?></option><?php
											} ?>
								</select>
						...
						<div class="lsrow">
							<div class="pluginChecker_multiSelect">
								...
								<select name="act_plugins_to[]" id="act_plugins_to" class="act_plugins_to" multiple="multiple">
									<option>Select here...</option><?php foreach ($deactivated_plugins as $key=>$value) {   ?><option value="<?php echo wp_kses_post($key); ?>"><?php echo wp_kses_post($value['Name']); ?></option><?php } ?>
								</select>
								<div class="pluginChecker_downArrow">
									<svg xmlns="http://www.w3.org/2000/svg" height="20" width="18" viewBox="0 0 448 512"><path opacity="1" fill="#1E3050" d="M201.4 342.6c12.5 12.5 32.8 12.5 45.3 0l160-160c12.5-12.5 12.5-32.8 0-45.3s-32.8-12.5-45.3 0L224 274.7 86.6 137.4c-12.5-12.5-32.8-12.5-45.3 0s-12.5 32.8 0 45.3l160 160z"/></svg>
								</div>
							...
							<div class="pluginChecker_multiSelect">
								...
								<select name="upd_plugins_to[]" id="upd_plugins_to" class="upd_plugins_to" multiple="multiple">
									<option>Select here...</option><?php foreach ($enabled_plugins as $key=>$value) {  ?>
											<option value="<?php echo wp_kses_post($key); ?>_<?php echo wp_kses_post($value['Version']); ?>"><?php echo wp_kses_post($value['Name']); ?></option><?php } ?>
								</select>
								<div class="pluginChecker_downArrow">
									<svg xmlns="http://www.w3.org/2000/svg" height="20" width="18" viewBox="0 0 448 512"><path opacity="1" fill="#1E3050" d="M201.4 342.6c12.5 12.5 32.8 12.5 45.3 0l160-160c12.5-12.5 12.5-32.8 0-45.3s-32.8-12.5-45.3 0L224 274.7 86.6 137.4c-12.5-12.5-32.8-12.5-45.3 0s-12.5 32.8 0 45.3l160 160z"/></svg>
								</div>
	...
	<div class="lpcBrand">
		<img src="<?php echo wp_kses_post( plugins_url( 'images/leidsenslogo.png', __FILE__ ) ); ?>" alt="">
	</div>
...
function LTPCD_leidsens_urlreplace_checker() { ?>
	<section class="lsbodyWrap">
		<div class="lsBrand">
					...
					<div class="col-md-6 pluginChecker_multiSelect_urlReplace">
						...
						<select name="leidsens_pages[]" id="leidsens_pages" class="leidsens_pages" multiple="multiple"><?php if( $my_posts->have_posts() ) {
									while ( $my_posts->have_posts() ) {
										$my_posts->the_post();
										?>
											<option value="<?php echo wp_kses_post(get_the_id()); ?>"><?php echo wp_kses_post(get_the_title()); ?></option><?php 		
									}
									wp_reset_postdata(); 
								}
							?>
	...
	if (isset($_POST['leidsens_replace'])) {
		...
		$searchStr = addslashes(esc_html(sanitize_text_field($_POST['leidsens_target_string'])));
		$replaceStr = addslashes(esc_html(sanitize_text_field($_POST['leidsens_replace_string'])));
		$selectedPostsForReplace = esc_html(sanitize_text_field($_POST['leidsens_replace_for']));
		
		if ($selectedPostsForReplace == 'leidsens_homes') {
			...
			foreach ($_POST[$selectedPostsForReplace] as $value) {
				$sanVal = sanitize_text_field($value);
				$escapeVal = esc_html($sanVal);
...
function LTPCD_leidsens_disable_editor(){
	?>
	<section class="lsbodyWrap">
	  	<div class="lsBrand">
		...
		if (isset($_POST['leidsence_act_deact_btn'])) {
			...
			if (esc_html(sanitize_text_field($_POST['leidsence_act_deact_value'])) == 'Deactivate') {
		...
		?>
		<h3 style="text-align: center;"><?php echo wp_kses_post($warningtxt) ?></h3>
		<div>
			<form action="" method="post" class="leidsens_disable_form">
				<div class="lsdbRow" style="margin-left: 38%;">
					<input type="hidden" name="leidsence_act_deact_value" value="<?php echo wp_kses_post($btn_val); ?>">
					<input type="submit" name="leidsence_act_deact_btn" class="leidsence_act_deact_btn" id="leidsence_act_deact_btn" value="<?php echo wp_kses_post($btn_val); ?> Theme and Plugin Editor">
				</div>
			</form>
		</div>
	</section>
...
function LTPCD_filter_plugin_checker_updates( $value ) {
	global $wpdb;
  	$table_name = $wpdb->prefix . "leidsens_plugin_checker_updates";
...
add_filter( 'site_transient_update_plugins', 'LTPCD_filter_plugin_checker_updates' ); ?>